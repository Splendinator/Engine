...
Shader::Shader(std::string vert, std::string frag, std::string geo)
{
	programID = glCreateProgram();
	GLuint vertObj = glCreateShader(GL_VERTEX_SHADER);
	GLuint fragObj = glCreateShader(GL_FRAGMENT_SHADER);
	GLuint geoObj = glCreateShader(GL_GEOMETRY_SHADER);
	char *vertText = IO::loadTextFromFile(vert);
	char *fragText = IO::loadTextFromFile(frag);
	char *geoText = IO::loadTextFromFile(geo);
	...
	//Geometry Shader
	glShaderSource(geoObj, 1, &geoText, NULL);
	glCompileShader(geoObj);
	glGetShaderiv(geoObj, GL_COMPILE_STATUS, &status);
	if (status != GL_TRUE) {
		glGetShaderInfoLog(geoObj, 8192, NULL, error);
		std::cout << "Error compiling geometry shader: (" << geo << ") " << error;
		programID = 0;
	}
	glAttachShader(programID, geoObj);
